...
void main() {
  ...
  test('issue 38', () async {
    ...
    String issuer = issuerKeyPair.accountId;
    await FriendBot.fundTestAccount(issuer);
    AccountResponse issuerAccount = await sdk.accounts.account(issuer);

    KeyPair keyPair = KeyPair.random();
    KeyPair anchorKeyPair = KeyPair.random();
    String senderId = keyPair.accountId;
    String withdrawAnchorAccount = anchorKeyPair.accountId;

    Transaction transaction = TransactionBuilder(issuerAccount)
        .addOperation(CreateAccountOperationBuilder(senderId, "10").build())
        .build();
    ...
    transaction = TransactionBuilder(issuerAccount)
        .addOperation(CreateAccountOperationBuilder(withdrawAnchorAccount, "10").build())
        .build();

    transaction.sign(issuerKeyPair, Network.TESTNET);

    response = await StellarSDK.TESTNET.submitTransaction(transaction);
    assert(response.success);

    AccountResponse sender = await StellarSDK.TESTNET.accounts.account(senderId);

    Asset asset = AssetTypeCreditAlphaNum4("SRT", issuer);

    ChangeTrustOperationBuilder chOp = ChangeTrustOperationBuilder(asset, "200999");

    transaction = TransactionBuilder(sender).addOperation(chOp.build()).build();
    ...
    AccountResponse anchorAccount = await StellarSDK.TESTNET.accounts.account(withdrawAnchorAccount);
    transaction = TransactionBuilder(anchorAccount).addOperation(chOp.build()).build();

    transaction.sign(anchorKeyPair, Network.TESTNET);

    response = await StellarSDK.TESTNET.submitTransaction(transaction);
    assert(response.success);

    transaction = TransactionBuilder(issuerAccount)
        .addOperation(PaymentOperationBuilder(senderId, asset, "100").build())
        .build();
    transaction.sign(issuerKeyPair, Network.TESTNET);
    ...
    bool found = false;
    sender = await StellarSDK.TESTNET.accounts.account(senderId);
    for (Balance? balance in sender.balances!) {
      if (balance!.assetType != Asset.TYPE_NATIVE && balance.assetCode == "SRT") {
        assert(double.parse(balance.balance!) > 90);
        found = true;
        break;
      }
    ...
    // send 50 SRT non native payment from C to B
    transaction = TransactionBuilder(sender)
        .addOperation(PaymentOperationBuilder(withdrawAnchorAccount, asset, "50").build())
        .addMemo(Memo.hash(Util.hexToBytes("000000000000000000000000000000002fbf727695e5431783cf6cbcef3864e1")))
        .build();
    transaction.sign(keyPair, Network.TESTNET);
    ...
    found = false;
    anchorAccount = await StellarSDK.TESTNET.accounts.account(withdrawAnchorAccount);
    for (Balance? balance in anchorAccount.balances!) {
      if (balance!.assetType != Asset.TYPE_NATIVE && balance.assetCode == "SRT") {
        assert(double.parse(balance.balance!) > 40);
        found = true;
        break;
