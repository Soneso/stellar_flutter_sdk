...
void main() {
  ...
  group('Util.decimalStringToStroops', () {
    test('converts decimal amount to stroops', () {
      final stroops = Util.decimalStringToStroops('100.5');
      expect(stroops, equals(BigInt.from(1005000000)));
    });

    test('converts integer amount', () {
      final stroops = Util.decimalStringToStroops('100');
    ...
    test('handles zero', () {
      final stroops = Util.decimalStringToStroops('0');
      expect(stroops, equals(BigInt.zero));
    });

    test('removes trailing zeros from decimal', () {
      final stroops = Util.decimalStringToStroops('100.5000000');
      expect(stroops, equals(BigInt.from(1005000000)));
    });

    test('handles maximum precision', () {
      final stroops = Util.decimalStringToStroops('1.1234567');
      expect(stroops, equals(BigInt.from(11234567)));
    });

    test('throws on too many decimal places', () {
      expect(() => Util.decimalStringToStroops('1.12345678'),
          throwsA(isA<Exception>()));
    });

    test('converts negative decimal to negative stroops', () {
      final stroops = Util.decimalStringToStroops('-1.5');
      expect(stroops, equals(BigInt.from(-15000000)));
    });

    test('converts negative sub-unit amount', () {
      final stroops = Util.decimalStringToStroops('-0.5');
      expect(stroops, equals(BigInt.from(-5000000)));
    });

    test('converts smallest negative amount', () {
      final stroops = Util.decimalStringToStroops('-0.0000001');
      expect(stroops, equals(BigInt.from(-1)));
    });
  });
  ...
  group('Util.stroopsToDecimalString', () {
    test('converts stroops to decimal amount', () {
      final amount = Util.stroopsToDecimalString(BigInt.from(1005000000));
      expect(amount, equals('100.5'));
    });

    test('converts integer stroops', () {
      final amount = Util.stroopsToDecimalString(BigInt.from(1000000000));
      expect(amount, equals('100'));
    });

    test('handles small amounts', () {
      final amount = Util.stroopsToDecimalString(BigInt.from(1));
      expect(amount, equals('0.0000001'));
    });

    test('handles zero', () {
      final amount = Util.stroopsToDecimalString(BigInt.zero);
    ...
    test('converts negative stroops to decimal', () {
      final amount = Util.stroopsToDecimalString(BigInt.from(-15000000));
      expect(amount, equals('-1.5'));
    });

    test('converts negative sub-unit stroops', () {
      final amount = Util.stroopsToDecimalString(BigInt.from(-5000000));
      expect(amount, equals('-0.5'));
    });

    test('converts smallest negative stroop', () {
      final amount = Util.stroopsToDecimalString(BigInt.from(-1));
      expect(amount, equals('-0.0000001'));
    });
    ...
    test('round trip conversion', () {
      ...
      final stroops = Util.decimalStringToStroops(original);
      final restored = Util.stroopsToDecimalString(stroops);
      expect(restored, equals(original));
    });

    test('negative round trip conversion', () {
      final original = '-123.456';
      final stroops = Util.decimalStringToStroops(original);
      final restored = Util.stroopsToDecimalString(stroops);
      expect(restored, equals(original));
  ...
  group('deprecated amount helpers delegate to the new methods', () {
    test('toXdrInt64Amount delegates to decimalStringToStroops', () {
      // ignore: deprecated_member_use_from_same_package
      expect(Util.toXdrInt64Amount('100.5'),
          equals(Util.decimalStringToStroops('100.5')));
    });

    test('fromXdrInt64Amount delegates to stroopsToDecimalString', () {
      // ignore: deprecated_member_use_from_same_package
      expect(Util.fromXdrInt64Amount(BigInt.from(1005000000)),
          equals(Util.stroopsToDecimalString(BigInt.from(1005000000))));
    });
  });
