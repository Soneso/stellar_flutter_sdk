...
void main() {
  String seed = "1123740522f11bfef6b3671f51e159ccf589ccf8965262dd5f97d1721d383dd4";

  test('test sign payload signer', () async {
      KeyPair keyPair = KeyPair.fromSecretSeedList(Uint8List.fromList(hex.decode(seed)));
      Uint8List payload = Uint8List.fromList([1,2,3,4,5]);
      XdrDecoratedSignature sig = keyPair.signPayloadDecorated(payload);
      assert(listEquals(sig.hint!.signatureHint, Uint8List.fromList([(0xFF & 252),65,0,50])));
  });
  ...
  test('test sign payload signer less than hint', () async {
    ...
    XdrDecoratedSignature sig = keyPair.signPayloadDecorated(payload);
    assert(listEquals(sig.hint!.signatureHint, Uint8List.fromList([255,64,7,55])));
  });

  test('it creates signed payload signer', () async {
    String accountStrKey = "GA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJVSGZ";
    String payloadStr = "0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20";
    Uint8List payload = Uint8List.fromList(base16decode(payloadStr));
    SignedPayloadSigner signedPayloadSigner = SignedPayloadSigner.fromAccountId(accountStrKey, payload);
    ...
    Uint8List b = signedPayloadSigner.signerAccountID.accountID!.getEd25519()!.uint256!;
  ...
  test('test valid signed payload encode', () async {
    ...
    String pEncoded = "PA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAQACAQDAQCQMBYIBEFAWDANBYHRAEISCMKBKFQXDAMRUGY4DUPB6IBZGM";
    ...
    signedPayloadSigner = SignedPayloadSigner.fromAccountId(accountStrKey, payload);
    encoded = StrKey.encodeSignedPayload(signedPayloadSigner);
    pEncoded = "PA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAOQCAQDAQCQMBYIBEFAWDANBYHRAEISCMKBKFQXDAMRUGY4DUAAAAFGBU";
    assert(encoded == pEncoded);
  ...
  test('test tx preconditions convert to xdr', () async {
    ...
    TransactionPreconditions cond2 = TransactionPreconditions.fromXdr(resultXdr)!;
