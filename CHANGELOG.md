## [Unreleased]
- SEP-10: `validateChallenge` now verifies the first operation's data value as required by the spec: it must be present, 64 bytes long, and base64-decode to a 48-byte nonce. A ManageData operation without a value now fails with a `ChallengeValidationError` (or `ChallengeValidationErrorInvalidWebAuthDomain` for the `web_auth_domain` operation) instead of an unhandled `TypeError`.
