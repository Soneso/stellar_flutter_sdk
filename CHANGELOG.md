## [Unreleased]
- SEP-29: `StellarSDK.submitTransaction`, `submitFeeBumpTransaction`, `submitAsyncTransaction`, `submitAsyncFeeBumpTransaction`, `submitTransactionEnvelopeXdrBase64` and `submitAsyncTransactionEnvelopeXdrBase64` run the SEP-29 memo-required check before submitting. A transaction without a memo that pays into, path-pays into or merges into an account whose `config.memo_required` data entry is `1` throws the new `AccountRequiresMemoException`, carrying the account id (`accountId`) and the zero-based index of the operation that names it (`operationIndex`), and nothing is submitted. The check performs one Horizon account lookup per distinct non-muxed destination, in operation order, and only when the transaction has no memo; multiplexed (`M...`) destinations are not looked up, and a destination Horizon does not know (404) is skipped, so the network decides about it. Fee bump transactions are checked through their inner transaction. The envelope-string methods decode the envelope to check it and submit an envelope they cannot decode unchecked. Other lookup failures surface from the submit call as `ErrorResponse` or `TooManyRequestsException`. Each method takes the named parameter `skipMemoRequiredCheck`; `skipMemoRequiredCheck: true` submits without the check. SEP-7's `URIScheme.signAndSubmitTransaction()` submits through the same methods and therefore also throws `AccountRequiresMemoException` for such a transaction.
- `StellarSDK.checkMemoRequired(transaction)` runs the same check without submitting: it completes when the transaction may be submitted and throws `AccountRequiresMemoException` otherwise.
- Subclasses that override one of the six submit methods must add the `skipMemoRequiredCheck` parameter, and classes that implement `StellarSDK` must also implement `checkMemoRequired`.
- `NetworkConstants.HTTP_NOT_FOUND` (404) joins the HTTP status constants.
- SEP-29 compatibility matrix: [compatibility/sep/SEP-0029_COMPATIBILITY_MATRIX.md](compatibility/sep/SEP-0029_COMPATIBILITY_MATRIX.md).
- Operations read from XDR render their `price` string exactly: `ManageBuyOfferOperation`, `ManageSellOfferOperation` and `CreatePassiveSellOfferOperation` built through `builder(...)`, and the `minPrice` and `maxPrice` of `LiquidityPoolDepositOperation`. The string is the decimal expansion of the XDR fraction, up to 20 decimal places and never in exponent notation. A price below 1e-6, such as 1/10000000, was rendered as `"1e-7"`, which `Price.fromString` refused when the operation was encoded again; it now reads `"0.0000001"`.
- `Price.fromString` parses a decimal exactly, as an integer over a power of ten, and expands it with integer arithmetic instead of through a `double`. Every nonzero price whose fraction in lowest terms, with a positive denominator, fits int32 round-trips through the operations above to that fraction: `"0.000000004096"` parses to 1/244140625 and `"0.99999999"` to 99999999/100000000, where the `double` expansion gave 1/244140627 and 1799999971/1799999989. A decimal that no int32 fraction carries exactly yields its closest int32 convergent. At the int32 boundaries the result follows the exact value, so a decimal just above 1/2147483648 parses to 1/2147483647 and a value below -2147483648 is refused. The accepted syntax, the int32 bounds and the exceptions are unchanged.
- `Price` gains `toDecimalString()`, the exact decimal rendering of the fraction described above. A zero denominator throws `ArgumentError`.
- `removeTailZero` returns a string without a decimal point unchanged. It previously removed trailing zeros from an integer as well, so `removeTailZero("100")` returned `"1"`.

## [3.7.0] - 15.Sep.2026.
- `Util.decimalStringToStroops` refuses an amount outside the int64 stroop range, throwing an `Exception` naming the value and the allowed range of -922337203685.4775808 to 922337203685.4775807. An amount past either bound previously encoded with only its low 64 bits kept, so `"922337203685.4775808"` (one stroop past the maximum) went out on the wire as `-922337203685.4775808`. The refusal reaches every operation that builds a stroop amount from a string in `toXdr()`: payments, path payments, offers, create account, change trust, clawback, claimable balances, and liquidity pool deposit and withdraw.
- `XdrDataOutputStream.writeBigInt64` throws `ArgumentError` for a value outside the union of the signed and unsigned 64-bit ranges, -2^63 to 2^64 - 1, instead of keeping only its low 64 bits. Both `XdrInt64` and `XdrUint64` encode through it, so the accepted range is their union; the exact signed bound for amounts is enforced in `Util.decimalStringToStroops`. `ArgumentError` is an `Error`, not an `Exception`, so an `on Exception catch` does not catch it.
- `Price.fromString` refuses a positive value whose fraction carries 308 to 322 leading zeros with the documented `Exception` ("Not a price an int32 fraction can carry") instead of leaking an `ArgumentError` from the fraction's reciprocal overflowing a double. A value with a non-zero whole part ahead of such a fraction now yields the whole part as the closest int32 fraction instead of throwing that `ArgumentError`.
- XDR definitions updated to stellar-xdr commit `c40231c`. The contract spec name limit is raised from 60 to 1024 bytes (the new `SC_SPEC_TYPE_NAME_LIMIT` constant in the XDR definitions) in `XdrSCSpecTypeUDT`, `XdrSCSpecUDTStructV0`, `XdrSCSpecUDTUnionV0`, `XdrSCSpecUDTEnumV0` and `XdrSCSpecUDTErrorEnumV0`, and `XdrSCSpecEventV0.name` is an XDR string under the same 1024-byte limit instead of an `SCSymbol`. All six `name` fields are `String` in Dart and their binary encoding is unchanged, so no API changes. The XDR-JSON readers and writers of the five UDT types accept names up to 1024 bytes where they previously refused more than 60, the event name is now capped at 1024 bytes, and the JSON keys and value shapes are unchanged.
...
- Compatibility matrices are generated against Horizon v28.0.1 and Soroban RPC v28.0.1. Coverage is unchanged at 100%; neither release adds an endpoint or method the SDK does not cover.
...
- `XdrSCAddress.forContractId`, `XdrSCAddress.forLiquidityPoolId` and `XdrLedgerKey.forLiquidityPool` refuse a hex id that does not render exactly 32 bytes, with a `FormatException` naming the id kind and the byte count given. A hex id of another width previously built an address or key over a hash of whatever width it spelled, so `'00ff'` produced a 2-byte hash where the protocol fixes 32. `XdrSCAddress.forContractId` also reads a 64-character hex contract id that begins with an uppercase `C` as hex: `'C'` is a hexadecimal digit, and the strkey branch is now gated on the exact strkey length, where such an id previously went into the strkey decoder and was refused. Lowercase hex was never affected.
...
- Create-contract host functions carry the external reference executable arm through the new `CreateContractFromExternalRefHostFunction` and `CreateContractFromExternalRefWithConstructorHostFunction` classes and the `XdrHostFunction.forCreatingContractWithExternalRef` and `forCreatingContractV2WithExternalRef` factories. `HostFunction.fromXdr` previously threw `UnimplementedError` for either external-reference create form; parsing an invoke-host-function operation through `InvokeHostFunctionOperation.builder`, including normal envelope parsing, now produces the corresponding host-function class.
- The new `SorobanClient.deployFromExternalRef` deploys a contract instance from a CAP-85 external reference; the new `DeployFromExternalRefRequest` names the owner contract and the tag instead of a wasm hash. The reference is resolved before the transaction is built, so an unresolvable reference throws an `Exception` naming the owner and the tag rather than failing on-chain; one message covers every miss, because `getExternalRefWasmHash` reports each of them as null. The contract spec is loaded from the resolved wasm before submission and the returned client is ready to invoke, the same flow `deploy` uses. Nothing is installed as part of the deployment. The operation always carries the `CREATE_CONTRACT_V2` arm with the constructor arguments, an empty vector when none are given, as `deploy` does; the plain `CREATE_CONTRACT` form stays available through `CreateContractFromExternalRefHostFunction`.
- Breaking change: the CAP-85 executable tag is carried as raw bytes. `XdrContractExecutableExternalRef.tag` is `Uint8List` and `XdrSCVal.executableTag` is `Uint8List?`, and `XdrContractExecutableExternalRef`'s positional constructor takes the tag as `Uint8List`. The new getters `tagString` and `executableTagString` read those bytes as UTF-8 and throw a `FormatException` when they spell no text. An executable tag is an XDR `string`, which carries arbitrary bytes, so a tag whose bytes are not valid UTF-8 is legal on the ledger; such a tag previously failed the whole ledger-entry decode, and there was no way to build one. `XdrSCVal.forExecutableTag(String)` and `XdrContractExecutable.forExternalRef(XdrSCAddress, String)` keep their signatures and encode the text as UTF-8; `XdrSCVal.forExecutableTagBytes` and `XdrContractExecutable.forExternalRefBytes` take the bytes. The wire, SEP-0051 and TxRep renderings of a tag that spells text are unchanged byte for byte.
- `CreateContractFromExternalRefHostFunction.tag` and `CreateContractFromExternalRefWithConstructorHostFunction.tag` are `Uint8List` and both classes gain a `tagString` getter and a `forTagString` constructor that takes the tag as text. `XdrHostFunction.forCreatingContractWithExternalRef` and `forCreatingContractV2WithExternalRef` take the tag as `Uint8List`. `DeployFromExternalRefRequest.tag` is likewise `Uint8List`, with the same `tagString` getter and a `forTagString` constructor that takes the tag as text and encodes it as UTF-8 exactly once. A tag decoded from the ledger reaches `SorobanServer.getExternalRefWasmHash` and the resolution inside `SorobanClient.deployFromExternalRef` as bytes, so the entry it resolves is the entry the reference names; an error naming a tag renders its bytes escaped.
...
- `Util.decimalStringToStroops`, which builds the stroop amount for the operations that take one, trims the value and validates its shape: after at most one leading sign, digits, then optionally a decimal point and any further digits; anything else throws an `Exception` naming the value. Whitespace around the value was not removed, so some padded amounts built a different amount than the caller wrote instead of being refused.
- `Price.fromString` validates the price string the same way: after the trim, at most one leading sign, digits, then optionally a decimal point and any further digits; anything else throws an `Exception` naming the value. `BigInt.parse` reads more than a decimal price, so `"0x10"` became the price 16/1, and a fraction is read only from a value in exactly two parts, so `"1.2.3"` became 1/1 with the remaining digits dropped — valid prices built from malformed input, so the operation went out at a price the caller did not ask for.
- `Price.fromString` approximates a negative price carrying a fraction to the right value: `-1.5` became `-1/2`, and a negative price smaller than one came out positive. The expansion now starts from the floor of the value instead of the part truncated towards zero, and its numerators are bounded to int32 from below (the new `BitConstants.INT32_MIN_VALUE`) as well as above, so the corrected fractions cannot wrap on the wire.
- `Price.fromString` refuses a value no int32 fraction can carry, throwing an `Exception` naming the value. A value beyond the int32 boundaries left the expansion at the invalid fraction 1/0, and zero or a value too small for any int32 fraction left it at 0/1; both encoded and were refused by the network instead of locally. A literal price of `"0"` therefore now throws during transaction building: `ManageSellOfferOperation`, `ManageBuyOfferOperation`, `CreatePassiveSellOfferOperation` and `LiquidityPoolDepositOperation` read their price strings through `Price.fromString` in `toXdr()`. Offer deletion is unaffected, since it carries amount zero with a valid price.
- SEP-6 and SEP-24 fee requests send the amount as a plain decimal. `SEP24FeeRequest` and `FeeRequest` carry the amount as a double that reached the query string through `toString`, which spells values below 1e-6 exponentially: one stroop went out as `amount=1e-7`. Amounts of everyday size came out right. The new `SepRequestAmount.format` renders from the shortest decimal representation that reads back to the same double, rounded to at most seven decimal places without an exponent, with a fraction ending exactly halfway rounding away from zero; the public double type is unchanged.
- `CreateContractFromExternalRefHostFunction` and `CreateContractFromExternalRefWithConstructorHostFunction` throw `ArgumentError` for an `executableOwner` that is not a contract address, at construction and on the setter. Only a contract can hold the executable tag entry, so such an operation can never execute; it previously encoded and failed on-chain. A transaction envelope carrying one is likewise refused when its operations are parsed into these classes; the raw XDR layer still parses it. `ArgumentError` is an `Error`, not an `Exception`, so an `on Exception catch` around construction does not catch it.
- Breaking change: CAP-71 upgraded authorization is the default. `SimulateTransactionRequest.useUpgradedAuth` and `MethodOptions.useUpgradedAuth` default to true, and the `useUpgradedAuth` key is always sent in the simulate request with the current value, so an explicit false reaches the server as the legacy opt-out; the key was previously omitted when false. `SorobanCredentials.forAddress` and `SorobanCredentials.forAddressCredentials` build the `ADDRESS_V2` arm; the new `forAddressLegacy` and `forAddressCredentialsLegacy` factories build the legacy `ADDRESS` arm, which the `SorobanCredentials(addressCredentials: ...)` constructor also still builds. RPC servers without protocol 27 support silently ignore the flag and return legacy entries, which stay valid. Set the flag to false on a network below protocol 27, where `ADDRESS_V2` entries invalidate the transaction. Auth entries returned by simulation against a protocol 27+ server carry the `ADDRESS_V2` arm under the default flag, and `SorobanCredentials.addressCredentials` is null on them; read the inner credentials of any address arm through `SorobanCredentials.innerAddressCredentials`, which the signing APIs already use.
- Breaking change: OpenZeppelin smart-account delegated external-wallet auth entries carry `ADDRESS_V2` credentials, so the preimage the wallet signs is `ENVELOPE_TYPE_SOROBAN_AUTHORIZATION_WITH_ADDRESS` and binds the wallet address; they previously carried the legacy `ADDRESS` arm. Setting the new `OZSmartAccountConfig.useUpgradedAuthForWalletSigners` (true by default) to false builds that legacy arm instead, for wallet software that cannot sign the address-bound preimage type. `fundWallet`'s source-account conversion likewise builds `ADDRESS_V2` credentials, carrying the temporary account's own address rather than the placeholder the legacy preimage allowed, and `OZSmartAccountAuth.buildSourceAccountAuthPayloadHash` takes that address as its second argument, because the address a signature covers has to be the address the credentials carry; it also gains a trailing named `useUpgradedAuth` parameter (default true) selecting the arm the hash is built for.
- `OZSmartAccountConfig.useUpgradedAuth` (true by default) governs the credential arm of the kit's internal simulations and of the `fundWallet` source-account conversion. Setting it to false makes every simulation the kit runs request legacy `ADDRESS` entries and the conversion write legacy `ADDRESS` credentials signed over `ENVELOPE_TYPE_SOROBAN_AUTHORIZATION`, so the submitted authorization XDR stays within the pre-protocol-27 schema, for relayer services that cannot parse protocol 27 XDR. It is separate from `OZSmartAccountConfig.useUpgradedAuthForWalletSigners`, which governs only the entries a delegated external wallet signs. Both flags carry a setter on `OZSmartAccountConfigBuilder` and participate in `OZSmartAccountConfig.copyWith`, `==` and `hashCode`.
- Compatibility matrices are generated against Horizon v28.0.0 and Soroban RPC v28.0.0 (Protocol 28). Coverage is unchanged at 100%; neither release adds an endpoint or method the SDK does not cover.
