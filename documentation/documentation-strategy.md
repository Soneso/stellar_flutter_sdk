...
Master's user-facing documentation and skill content describe the released SDK, because users read them on GitHub against the SDK they can install. Documentation and skill content for unreleased behavior goes to the `release-docs` branch, not to the feature branch that implements the behavior; `release-docs` is merged as the first step of release preparation. Feature branches carry only code, tests (including doc tests), the CHANGELOG's `[Unreleased]` section, and version-independent process documents. Skill archives are rebuilt only during release preparation, never on feature branches.
