...
Policies can also be installed at deploy time on the wallet's default rule, instead of being added afterward. Set `OZSmartAccountConfig.defaultPolicies`, or pass a per-call `policies` map to `createWallet` / `deployPendingCredential`, keyed by policy contract address with the policy's install parameters (an `OZPolicyInstallParams`) as the value. The kit passes them through the contract constructor, so the new wallet starts with those policies already enforced; a per-call argument overrides the config default. Because the default rule starts with a single signer and the spending-limit policy only installs on call-contract rules, this is primarily useful for a threshold of 1 (which keeps the rule at 1-of-N as more signers are added) or custom policies.
