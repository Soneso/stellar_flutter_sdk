...
- Android 9.0 (API level 28) or newer, required by Credential Manager's passkey surface.
...
API 28 is required by the Credential Manager passkey path. Setting `minSdk` lower than 28 will fail at runtime the first time the SDK tries to register or authenticate a passkey.
...
- **`OZPlatformStorageAdapter`**: production storage backed by `EncryptedSharedPreferences` (values encrypted with AES-256-GCM, keys wrapped with AES-256-SIV via the Android Keystore). Read-modify-write sequences are serialised on the native side.
