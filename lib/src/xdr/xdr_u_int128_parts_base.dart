// Copyright 2026 The Stellar Flutter SDK Authors. All rights reserved.
// Use of this source code is governed by a license that can be
// found in the LICENSE file.
// This file is automatically generated by xdrgen. Do not edit manually.

import 'dart:convert';
import 'dart:typed_data';

import 'xdr_data_io.dart';
import 'xdr_json_helper.dart';
import 'xdr_uint64.dart';

class XdrUInt128PartsBase {
  XdrUint64 _hi;
  XdrUint64 get hi => this._hi;
  set hi(XdrUint64 value) => this._hi = value;

  XdrUint64 _lo;
  XdrUint64 get lo => this._lo;
  set lo(XdrUint64 value) => this._lo = value;

  XdrUInt128PartsBase(this._hi, this._lo);

  static void encode(
    XdrDataOutputStream stream,
    XdrUInt128PartsBase encodedUInt128Parts,
  ) {
    XdrUint64.encode(stream, encodedUInt128Parts.hi);
    XdrUint64.encode(stream, encodedUInt128Parts.lo);
  }

  static XdrUInt128PartsBase decode(XdrDataInputStream stream) {
    XdrUint64 hi = XdrUint64.decode(stream);
    XdrUint64 lo = XdrUint64.decode(stream);
    return XdrUInt128PartsBase(hi, lo);
  }

  String toBase64EncodedXdrString() {
    XdrDataOutputStream xdrOutputStream = XdrDataOutputStream();
    XdrUInt128PartsBase.encode(xdrOutputStream, this);
    return base64Encode(xdrOutputStream.bytes);
  }

  static XdrUInt128PartsBase fromBase64EncodedXdrString(String base64Encoded) {
    Uint8List bytes = base64Decode(base64Encoded);
    return XdrUInt128PartsBase.decode(XdrDataInputStream(bytes));
  }

  void toTxRep(String prefix, List<String> lines) {
    _hi.toTxRep('$prefix.hi', lines);
    _lo.toTxRep('$prefix.lo', lines);
  }

  static XdrUInt128PartsBase fromTxRep(Map<String, String> map, String prefix) {
    XdrUint64 hi = XdrUint64.fromTxRep(map, '$prefix.hi');
    XdrUint64 lo = XdrUint64.fromTxRep(map, '$prefix.lo');
    return XdrUInt128PartsBase(hi, lo);
  }

  /// Returns the SEP-0051 XDR-JSON rendering of this value.
  String toXdrJson() =>
      XdrJsonHelper.encodeDocument(toXdrJsonValue(), type: 'XdrUInt128Parts');

  /// Parses the SEP-0051 XDR-JSON rendering of a XdrUInt128Parts.
  static XdrUInt128PartsBase fromXdrJson(String json) => fromXdrJsonValue(
    XdrJsonHelper.decodeDocument(json, type: 'XdrUInt128Parts'),
  );

  /// Returns the SEP-0051 rendering of this XdrUInt128Parts.
  Object? toXdrJsonValue() => XdrJsonHelper.partsToDecimalString(
    <BigInt>[_hi.uint64, _lo.uint64],
    signed: false,
    type: 'XdrUInt128Parts',
  );

  /// Reads a XdrUInt128Parts from its SEP-0051 rendering.
  static XdrUInt128PartsBase fromXdrJsonValue(Object? value) {
    final List<BigInt> limbs = XdrJsonHelper.decimalStringToParts(
      value,
      limbCount: 2,
      signed: false,
      type: 'XdrUInt128Parts',
    );
    return XdrUInt128PartsBase(XdrUint64(limbs[0]), XdrUint64(limbs[1]));
  }
}
