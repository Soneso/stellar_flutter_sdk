// Copyright 2026 The Stellar Flutter SDK Authors. All rights reserved.
// Use of this source code is governed by a license that can be
// found in the LICENSE file.
// This file is automatically generated by xdrgen. Do not edit manually.

import 'dart:convert';
import 'dart:typed_data';

import 'xdr_data_io.dart';
import 'xdr_json_helper.dart';

class XdrSurveyMessageResponseType {
  final _value;
  const XdrSurveyMessageResponseType._internal(this._value);
  toString() => 'SurveyMessageResponseType.$_value';
  XdrSurveyMessageResponseType(this._value);
  get value => this._value;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is XdrSurveyMessageResponseType && _value == other._value;

  @override
  int get hashCode => _value.hashCode;

  static const SURVEY_TOPOLOGY_RESPONSE_V2 =
      const XdrSurveyMessageResponseType._internal(2);

  static XdrSurveyMessageResponseType decode(XdrDataInputStream stream) {
    int value = stream.readInt();
    switch (value) {
      case 2:
        return SURVEY_TOPOLOGY_RESPONSE_V2;
      default:
        throw Exception("Unknown enum value: $value");
    }
  }

  static void encode(
    XdrDataOutputStream stream,
    XdrSurveyMessageResponseType value,
  ) {
    stream.writeInt(value.value);
  }

  String toBase64EncodedXdrString() {
    XdrDataOutputStream xdrOutputStream = XdrDataOutputStream();
    XdrSurveyMessageResponseType.encode(xdrOutputStream, this);
    return base64Encode(xdrOutputStream.bytes);
  }

  static XdrSurveyMessageResponseType fromBase64EncodedXdrString(
    String base64Encoded,
  ) {
    Uint8List bytes = base64Decode(base64Encoded);
    return XdrSurveyMessageResponseType.decode(XdrDataInputStream(bytes));
  }

  /// Returns the SEP-0051 XDR-JSON rendering of this value.
  String toXdrJson() => XdrJsonHelper.encodeDocument(
    toXdrJsonValue(),
    type: 'XdrSurveyMessageResponseType',
  );

  /// Parses the SEP-0051 XDR-JSON rendering of a XdrSurveyMessageResponseType.
  static XdrSurveyMessageResponseType fromXdrJson(String json) =>
      fromXdrJsonValue(
        XdrJsonHelper.decodeDocument(
          json,
          type: 'XdrSurveyMessageResponseType',
        ),
      );

  /// Returns this member's SEP-0051 name.
  Object? toXdrJsonValue() {
    switch (_value) {
      case 2:
        return 'survey_topology_response_v2';
      default:
        XdrJsonHelper.fail(
          'XdrSurveyMessageResponseType',
          'holds the unknown value $_value',
        );
    }
  }

  /// Reads a XdrSurveyMessageResponseType from its SEP-0051 name.
  static XdrSurveyMessageResponseType fromXdrJsonValue(Object? value) {
    if (value is String) {
      switch (value) {
        case 'survey_topology_response_v2':
          return XdrSurveyMessageResponseType.SURVEY_TOPOLOGY_RESPONSE_V2;
      }
    }
    XdrJsonHelper.fail(
      'XdrSurveyMessageResponseType',
      'expects one of its member names but found ${XdrJsonHelper.preview(value)}',
    );
  }
}
