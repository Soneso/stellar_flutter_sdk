...
#
# corpus.json carries no timestamp and a fixed entry and key order, so the whole
# document is compared raw and any difference is real drift.
#
# Usage:
...
#      advisory mode, or a committed artefact the generator reads is absent)
...
# Prerequisites first, before this script creates or writes anything at all. A
# missing CLI must never be mistakable for a clean diff, and must never leave a
# half-written artefact behind.
python3 "$SCRIPT_DIR/generate_corpus.py" $ADVISORY --check-prerequisites || exit 2

if [ ! -f "$COMMITTED" ]; then
  echo "refresh_corpus.sh: $COMMITTED does not exist." >&2
  echo "  Create it with: python3 $SCRIPT_DIR/generate_corpus.py" >&2
  exit 1
fi
...
echo "  Then re-emit the embedded copy: make sep51-generate-tests" >&2
exit 1
