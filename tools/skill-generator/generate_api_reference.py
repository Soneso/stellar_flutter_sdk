...
def github_anchor(heading: str) -> str:
    """
    Slugify a heading the way GitHub does: lowercase, drop punctuation (keeping
    word characters, hyphens and spaces), then spaces to hyphens.
    """
...
def render_markdown(groups: dict[str, list[ClassInfo]], stats: dict) -> str:
    ...
    emitted = [(key, title) for key, title in GROUP_TITLES.items() if groups[key]]
    ...
    toc = " | ".join(f"[{title}](#{github_anchor(title)})" for _key, title in emitted)
    parts.append(f"**Sections:** {toc}\n\n")

    for group_key, title in emitted:
        parts.append("---\n")
